...
\[usbdrive\] comment = external drive path = /mnt/usbdrive valid users = @users force group = users create mask = 0660 directory mask = 0771 read only = no
...
![[attachments/Synology_DiskStation_-_vdSarDisk_3.png]]

Add a new task brings the following screen:

![[attachments/Synology_DiskStation_-_vdSarDisk.png]][
...
Install Fail2Ban to block connections from IP addresses that have more than x-number of failed login attempts. 
